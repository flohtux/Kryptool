...
public class SimpleFileGUI extends JFrame  implements DropTargetListener,ProgressObsever{
	...
	private final JComboBox secLevel;
	...
	public SimpleFileGUI() {
		...
		JLabel lblSicherheitsstufe = new JLabel("Sicherheitsstufe");
		GridBagConstraints gbc_lblSicherheitsstufe = new GridBagConstraints();
		gbc_lblSicherheitsstufe.anchor = GridBagConstraints.EAST;
		gbc_lblSicherheitsstufe.insets = new Insets(0, 0, 5, 5);
		gbc_lblSicherheitsstufe.gridx = 0;
		gbc_lblSicherheitsstufe.gridy = 1;
		contentPane.add(lblSicherheitsstufe, gbc_lblSicherheitsstufe);
		
		secLevel = new JComboBox(SecLevel.values());
		secLevel.addItemListener(new ItemListener() {
			public void itemStateChanged(ItemEvent e) {
				GuiManager.getInstance().setSecLevel(SecLevel.valueOf(SimpleFileGUI.this.secLevel.getSelectedItem().toString()));
			}
		});
		GridBagConstraints gbc_secLevel = new GridBagConstraints();
		gbc_secLevel.insets = new Insets(0, 0, 5, 0);
		gbc_secLevel.fill = GridBagConstraints.HORIZONTAL;
		gbc_secLevel.gridx = 1;
		gbc_secLevel.gridy = 1;
		contentPane.add(secLevel, gbc_secLevel);
		...
		gbc_btnVerschlsseln.gridwidth = 2;
		...
		gbc_btnEntschlsseln.gridwidth = 2;
	...
	private void updateSecLevel() {
		...
		if(p.bitLength() < 129){
			this.secLevel.setSelectedItem(SecLevel.niedrig);
			GuiManager.getInstance().setSecLevel(SecLevel.niedrig);
		} else if(p.bitLength() < 1025){
			this.secLevel.setSelectedItem(SecLevel.mittel);
			GuiManager.getInstance().setSecLevel(SecLevel.mittel);
		} else {
			this.secLevel.setSelectedItem(SecLevel.hoch);
			GuiManager.getInstance().setSecLevel(SecLevel.hoch);
		}
	}
